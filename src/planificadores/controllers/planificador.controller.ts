...
export class PlanificadorController {
  ...
  async findAll(req: Request, res: Response) {
    try {
        ...
        const planificadors = await this.planificadorService.findAll(paginationDto!);
  ...
  async findOne(req: Request, res: Response) {
    try {
        const planificador = await this.planificadorService.findOne(req.params.id);
  ...
  async update(req: Request, res: Response) {
    try {
        ...
        const planificador = await this.planificadorService.update(req.params.id, updDto!);
        return res.status(200).json(planificador);
    }
    catch (error) {
        handleRestExceptions(error, res);
